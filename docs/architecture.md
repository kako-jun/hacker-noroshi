...
| item_type | TEXT | 'story', 'comment', or 'poll_option' |
...
投票（poll, #74）の選択肢を保持する。1 つの type='poll' な stories 行に対して 2〜10 行が並ぶ。

| カラム | 型 | 備考 |
|---|---|---|
| id | INTEGER PK | autoincrement |
| story_id | INTEGER FK | type='poll' な stories.id |
| text | TEXT | 選択肢テキスト（最大 300 文字） |
| position | INTEGER | 表示順 |
| created_at | TEXT | ISO8601 |
| INDEX | idx_poll_options_story ON (story_id) | 一括取得用 |

選択肢への投票は `votes(item_type='poll_option', vote_type='up')` として記録する。
karma は加算しない（本家HN 準拠）。
...
active な ban を全削除するセルフサービス導線を持つ。
env: `TURNSTILE_SITE_KEY`（public）と `TURNSTILE_SECRET_KEY`（secret）。
ソフト制限は cookie `unban_attempts` で 24h / 3 回。
詳細は `docs/spec.md` の「セルフサービス unban (#91)」と
`docs/operations.md` の「#91 セルフサービス unban（本番反映手順）」を参照。
...
| `/submit` | 投稿フォーム（要ログイン） |
| `/newpoll` | 投票（poll）投稿フォーム（要ログイン、#74） |
| `/polls` | type='poll' 一覧（#74） |
...
| `getPollOptionById()` | poll_option の存在確認 + 紐づく story_id（vote API のバリデーション、#74） |
| `getPolls()` | type='poll' の stories 一覧（points 降順、#74） |
