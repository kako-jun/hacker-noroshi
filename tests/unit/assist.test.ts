...
describe('assistHint の和名併記とロケール分離', () => {
	...
	// #172 must 2: /user/[id]/hidden の un-hide ボタンに hide の説明（逆の意味）が出ていた事故の回帰ガード。
	// story.hide と story.un-hide は文言が入れ替わっても検知できるよう、互いに含んではいけない文言も見る。
	it("story.un-hide(ja) は『非表示解除（un-hide）』を併記し、story.hide の文言（自分の一覧から消します）は含まない", () => {
		const hint = assistHint('story.un-hide', 'ja');
		expect(hint).toContain('非表示解除（un-hide）');
		expect(hint).not.toContain('この投稿を自分の一覧から消します');
	});

	it("story.hide(ja) は story.un-hide の文言（この一覧に戻し）を含まない", () => {
		expect(assistHint('story.hide', 'ja')).not.toContain('この一覧に戻し');
	});

	it("item.upvote(ja) は story.upvote と同じ『upvote（投票）』の説明を持つ", () => {
		expect(assistHint('item.upvote', 'ja')).toContain('▲ は upvote（投票）');
	});
...
describe('assistHint はコントロール要素単位に分割されている（#172）', () => {
	const granularKeys = [
		...
		'item.comment-toggle',
		'item.reply',
		// #175: /comment/[id] のアクション行（upvote/edit/flag/vouch）用に追加した granular キー。
		'comment.upvote',
		'comment.edit',
		'comment.flag',
		'comment.vouch'
...
/**
 * D. #175: /comment/[id]（コメント単独表示）のアクション行専用キー（comment.upvote/edit/flag/vouch）。
 * item.* と対になる新設キー群。item.* との対称性（U1）・コピペ取り違え回帰（U2）・ロケール分離（U3）を見る。
 */
describe('assistHint の comment.* キー（#175: /comment/[id] アクション行）', () => {
	// U1: item.* 側と対称に、ja は「和名（英語）」併記を持つ。
	it('comment.edit/flag/vouch(ja) はそれぞれ『和名（英語）』を併記する（item.* 側との対称性）', () => {
		expect(assistHint('comment.edit', 'ja')).toContain('編集（edit）');
		expect(assistHint('comment.flag', 'ja')).toContain('通報（flag）');
		expect(assistHint('comment.vouch', 'ja')).toContain('復活（vouch）');
	});

	// U2（事故回帰ガード）: #172 must-2 (story.hide/un-hide 取り違え) と同型のコピペ取り違え検出。
	// comment.upvote/edit は「コメント」を指す文言でなければならず、item.* 側の「投稿」語を含んではいけない。
	it('comment.upvote/edit(ja) は「コメント」を指し、item.* の「投稿」文言を含まない（コピペ取り違え回帰ガード）', () => {
		const upvote = assistHint('comment.upvote', 'ja');
		expect(upvote).toContain('コメント');
		expect(upvote).not.toContain('投稿');

		const edit = assistHint('comment.edit', 'ja');
		expect(edit).toContain('コメント');
		expect(edit).not.toContain('投稿');
	});

	// U3: en 側はロケール分離を維持し、日本語を含まない。
	it('comment.upvote/edit/flag/vouch(en) は英語のまま＝日本語を含まない（ロケール分離）', () => {
		const keys = ['comment.upvote', 'comment.edit', 'comment.flag', 'comment.vouch'] as const;
		for (const key of keys) {
			const hint = assistHint(key, 'en');
			expect(hint).not.toBe('');
			expect(hint).not.toContain('コメント');
			expect(hint).not.toContain('編集');
			expect(hint).not.toContain('通報');
			expect(hint).not.toContain('復活');
		}
	});
});
