...
- **Control hints sit next to their control.** Form fields keep inline `.assist-hint`. Concept hints that would otherwise repeat per row appear once: `story.controls` renders only on the first list row (`StoryListItem` when `rank === 1`); `item.controls` renders once under the item's action line; `meta.controls` renders once under the header (teaches the top-right `lang` toggle, the `(123)` karma, and the assist switch itself).
