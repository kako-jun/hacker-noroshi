...
- **Control hints sit next to their control — one button, one hint (#172).** Each control has its own single-sentence hint keyed per element (e.g. `story.upvote`/`story.hide`/`story.flag`/`story.comments`, `item.favorite`/`item.hide`/`item.flag`/`item.edit`/`item.delete`/`item.comment-toggle`/`item.reply`), not one paragraph bundling several concepts under a row. Form fields keep inline `.assist-hint`; row/action controls wrap their target in `.assist-anchor` (`position: relative`) and float the hint directly beneath it via `.assist-hint-float` (`.assist-stagger-1..4` / `.assist-stagger-below-meta` offset variants prevent same-line hints from overlapping when several controls are close together, e.g. edit/delete/hide/favorite/flag on `/item`). Hints that would otherwise repeat per row appear once: the story-row hints render only on the first *rendered* list row (`StoryList` passes `assistFirst` to the first non-hidden story, so it still shows on page 2, on `/search` where no rank is passed, and after the top row is hidden); the item-comment hints (`item.comment-toggle`/`item.reply`) render once on the first comment row.
